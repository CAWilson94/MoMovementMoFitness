...
function normaliseSession(session) {
  const type = session.type === "Workout" || session.type === "WeightTraining" ? "gym" : session.type;
  const normalType = type === "Run" || type === "run" ? "run" : type === "gym" ? "gym" : type;
  return {
    id: session.id || `${session.date}-${session.title}`,
    title: session.title || (normalType === "gym" ? "Gym session" : "Run"),
    date: session.date,
    type: normalType,
...
async function loadSessions() {
  ...
  const sessions = [...stravaSessions, ...manualSessions]
    .filter((session) => ["run", "gym"].includes(session.type))
...
function renderCurrentWeek(sessions, weeks) {
  ...
  selectors.weekGym.textContent = `${gym}/${activeWeek.gymTarget}`;
  selectors.weekRuns.textContent = `${runs}/${activeWeek.runTarget}`;
  selectors.weekTotal.textContent = `${total}/${activeWeek.gymTarget + activeWeek.runTarget}`;
...
function renderWeeks(sessions, weeks) {
  ...
  for (const week of weeks) {
    ...
    card.innerHTML = `
      ...
      <div class="week-dots" aria-label="${gym} gym sessions and ${runs} runs logged">
        ${dotMarkup(gym, 2, "gym")}
...
function renderActivities(sessions) {
  ...
  selectors.activitySummary.textContent = `${sessions.length} sessions in this block`;

  if (!sessions.length) {
    selectors.activityList.innerHTML = `<article class="activity-card"><h3>The log is empty</h3><p>Add gym sessions manually or let Strava fetch the runs.</p></article>`;
    return;
  }
  ...
  for (const session of sessions.slice(0, 9)) {
    ...
    const detail =
      session.type === "run" && session.distanceMiles
